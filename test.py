...
image_data_loader = data.DataLoader(
    ...
    shuffle=False,#originally true, changed to false for consistency
...
with torch.no_grad():
    ...
    for _ in tqdm(range(opts.number_eval)):
        ...
        save_image(output, opts.result_root + '/{:05d}.jpg'.format(_))
        save_image(output_comp, opts.result_root + '/{:05d}.png'.format(_))
        #save_image(ground_truth, opts.result_root + 'real/{:05d}.png'.format(_))
        #modified to also save the original image
